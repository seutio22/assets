...
interface ContatoSubEstipulanteFormProps {
  contatoId?: string
...
const ContatoSubEstipulanteForm = ({ contatoId, subEstipulanteId, onSuccess, onCancel }: ContatoSubEstipulanteFormProps) => {
  ...
  const fetchContato = async () => {
    try {
      const response = await api.get(`/contatos-sub-estipulante/${contatoId}`)
      const contato = response.data
      setFormData({
        subEstipulanteId: contato.subEstipulanteId,
        nome: contato.nome,
        cargo: contato.cargo || '',
        email: contato.email || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (contatoId) {
        await api.put(`/contatos-sub-estipulante/${contatoId}`, data)
      } else {
        await api.post('/contatos-sub-estipulante', data)
      }
      onSuccess()
    } catch (err: any) {
...
export default ContatoSubEstipulanteForm
